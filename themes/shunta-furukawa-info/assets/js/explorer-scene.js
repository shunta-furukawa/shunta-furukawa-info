...
import {buildHarbor,createArtifact,whiteBoundary,artifactImages,setArtifactColor} from './harbor-models.js';
...
export function createExplorer(host,{onNear,onRead,onPosition,onFailure,onImages=()=>{},onCoin=()=>{},reduced=false}) {
 ...
 let accent="#ff3b8d";const coinMeshes=new Map(),coinGeo=new T.TorusGeometry(.32,.085,10,24);let pickedCoins=new Set();for(const c of COINS){const m=new T.Mesh(coinGeo,solids.pink);m.position.set(c.x,.8,c.z);environment.add(m);coinMeshes.set(c.id,m);}
 ...
 for(const sign of SIGNS){box(environment,sign.x,1,sign.z,.22,2,.3,'white');const signMaterial=label(sign.title,sign.short);const board=new T.Mesh(new T.BoxGeometry(3.2,1.1,.18),[solids.dark,solids.dark,solids.dark,solids.dark,signMaterial,signMaterial]);board.position.set(sign.x*WORLD_SCALE,2.15,sign.z*WORLD_SCALE);board.rotation.y=signYaw(sign);environment.add(board);boards.set(sign.id,board);const artifact=createArtifact(sign.model);artifact.position.set(sign.x*WORLD_SCALE,3.65,sign.z*WORLD_SCALE);artifact.scale.setScalar(.9);environment.add(artifact);exhibits.set(sign.id,artifact);
  if(sign.id!=='profile'){const device=createArtifact(sign.model);device.position.set(sign.x*WORLD_SCALE+Math.cos(board.rotation.y)*2.6,1.05,sign.z*WORLD_SCALE-Math.sin(board.rotation.y)*2.6);device.rotation.y=board.rotation.y;device.scale.setScalar(1.15);environment.add(device);const light=new T.Mesh(new T.SphereGeometry(.14,16,12),solids.gray);light.position.copy(device.position).add(new T.Vector3(0,1.2,0));environment.add(light);devices.set(sign.id,{device,light,baseY:1.05,angle:board.rotation.y});}
 ...
 function updateExhibits(){for(const sign of SIGNS){const owned=collected.has(sign.id),seen=visited.has(sign.id),board=boards.get(sign.id),old=board.material[4];old.map.dispose();old.dispose();const material=label(sign.title,sign.short,owned?'稼働中 ✓':seen?'未修復 · 調べて直す':'近づいて調べる');board.material[4]=material;board.material[5]=material;exhibits.get(sign.id).visible=!owned;badges.get(sign.id).visible=owned;if(devices.has(sign.id))devices.get(sign.id).light.material=owned?solids.pink:solids.gray;}}
 ...
 let jumpState={phase:'ground',y:0,v:0};function jump(){if(!reading&&jumpState.phase==='ground')jumpState={phase:'launch',y:0,v:0,delay:.09};}
 ...
 function draw(dt){time+=motionReduced?0:dt;
  ...
  if(!reading)jumpState=jumpStep(jumpState,dt);player.position.y=jumpState.y;
  ...
  for(const c of COINS){const m=coinMeshes.get(c.id);m.visible=!pickedCoins.has(c.id);m.rotation.y=motionReduced?0:time*1.4;if(m.visible&&!reading&&Math.hypot(c.x-player.position.x,c.z-player.position.z)<1.05&&player.position.y<1.5){pickedCoins.add(c.id);m.visible=false;onCoin(c.id);}}
 ...
 return {jump,setCoins(ids){pickedCoins=new Set(ids);},setAccent(color){accent=color;solids.pink.color.set(color);mats.pink.forEach(m=>m.color.set(color));setArtifactColor(color);updateExhibits();try{onImages(artifactImages(SIGNS));}catch{}},restore(s){clearInput();const p=obstacles.some(o=>Math.hypot(o.x-s.position.x,o.z-s.position.z)<o.r+.5)?SPAWN:s.position;player.position.set(p.x,0,p.z);player.rotation.y=s.yaw;yaw=s.yaw+Math.PI;jumpState={phase:'ground',y:0,v:0};first=true;},setCollected(ids){if(ids.length===collected.size&&ids.every(id=>collected.has(id)))return;collected=new Set(ids);updateExhibits();},setVisited(ids){visited=new Set(ids);updateExhibits();},getNearbyId(){return nearestSign(player.position)?.id||null;},setStick(x,z){stick={x,z};},clearInput,setReading(v){reading=v;clearInput();},interact,setReduced(v){motionReduced=v;},reset(){jumpState={phase:'ground',y:0,v:0};clearInput();player.position.set(SPAWN.x,0,SPAWN.z);yaw=0;pitch=0;player.rotation.y=Math.PI;manualUntil=0;first=true;},rotate(amount){manualCamera();yaw+=amount;},setRun(v){running=v;},setSimulate(v){simulation=v;},setConversation(v){conversation=v;},focus(){canvas.focus({preventScroll:true});}};
}
