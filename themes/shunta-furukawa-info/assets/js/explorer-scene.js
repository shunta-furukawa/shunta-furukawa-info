...
export function createExplorer(host,{onNear,onRead,onPosition,onFailure,onImages=()=>{},onCoin=()=>{},onFall=()=>{},reduced=false}) {
 ...
 let accent="#ff3b8d";const coinMeshes=new Map(),coinGeo=new T.TorusGeometry(.32,.085,10,24);let pickedCoins=new Set();for(const c of COINS){const m=new T.Mesh(coinGeo,solids.pink);m.position.set(c.x,c.y,c.z);environment.add(m);coinMeshes.set(c.id,m);}
 const riverShape=new T.Shape();for(let i=0;i<=100;i++){const x=RIVER.start+(RIVER.end-RIVER.start)*i/100,z=riverCenter(x)-RIVER.halfWidth;i?riverShape.lineTo(x,z):riverShape.moveTo(x,z);}for(let i=100;i>=0;i--){const x=RIVER.start+(RIVER.end-RIVER.start)*i/100;riverShape.lineTo(x,riverCenter(x)+RIVER.halfWidth);}riverShape.closePath();const water=new T.Mesh(new T.ShapeGeometry(riverShape),new T.MeshBasicMaterial({color:0x101d2b,side:T.DoubleSide}));water.rotation.x=Math.PI/2;water.position.y=.045;environment.add(water);
 const bankMaterial=new T.MeshBasicMaterial({color:0xb7beca});for(const side of [-1,1]){const points=[];for(let i=0;i<=100;i++){const x=RIVER.start+(RIVER.end-RIVER.start)*i/100;points.push(new T.Vector3(x,.075,riverCenter(x)+side*(RIVER.halfWidth+.12)));}const bank=new T.Mesh(new T.TubeGeometry(new T.CatmullRomCurve3(points),100,.055,6,false),bankMaterial);environment.add(bank);}
 ...
 function draw(dt){time+=motionReduced?0:dt;
  ...
  for(const c of COINS){const m=coinMeshes.get(c.id);m.visible=!pickedCoins.has(c.id);m.rotation.y=motionReduced?0:time*1.4;if(m.visible&&!reading&&canReachCoin(player.position,c)){pickedCoins.add(c.id);m.visible=false;onCoin(c.id);}}
  ...
  renderer.render(world,camera);onPosition({safe:!waterAt(player.position.x,player.position.z)&&jumpState.phase==='ground',x:player.position.x,z:player.position.z,avatarYaw:player.rotation.y,near:near?.id});
 }
